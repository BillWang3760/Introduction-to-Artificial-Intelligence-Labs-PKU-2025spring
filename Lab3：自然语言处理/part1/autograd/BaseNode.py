...
class Attention(Node):
    ...
    def cal(self, X):
        ...
        K = X @ W_k  # (B, L, d)
        V = X @ W_v  # (B, L, d)
        score = (Q @ K.transpose(0, 2, 1)) / np.sqrt(self.dim)  # (B, L, L)
        score = score - np.max(score, axis=-1, keepdims=True)
        exp_score = np.exp(score)
        softmax_score = exp_score / exp_score.sum(axis=-1, keepdims=True)
...
class ResLinear(Node):
    ...
    def backcal(self, grad):
        # TODO: YOUR CODE HERE
        # shape grad: (B, L, d)
        X = self.cache[-1]  # （B, L, d)
        weight_grad = (X.transpose(0, 2, 1) @ grad).sum(axis=0)
        self.grad.append(weight_grad)
        bias_grad = grad.sum(axis=0).sum(axis=0)
        self.grad.append(bias_grad)
        X_grad = grad @ self.params[0].T
        return X_grad + grad
